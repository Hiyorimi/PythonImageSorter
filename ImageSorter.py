...
class ImagePreview:
    def __init__(self, root):
        ...
        self.imageContainer = Label(self.imageWindow, image=None, text="Press space to move to folder. Press Enter to continue.", height=51)
...
class ImageSorterApp:
    def __init__(self):
        ...
        self.root.bind("<Return>", self.classifyImage)
        self.root.bind("<space>", self.cycleImage)
    ...
    def transferImage(self, imageName):
        ...
        if imageName != "None" and imageName != None:
            ...
            for dirName in copyDirs:
                shutil.copy2(self.cwd[:-1] + '/' + imageName, self.cwd[:-1] + '/' + dirName)
                print self.cwd[:-1] + '/' + imageName, self.cwd[:-1] + '/' + dirName
                # Verify that the size of the image remained the same
                if os.stat(self.cwd[:-1] + '/' + imageName).st_size != os.stat(self.cwd[:-1] + '/' + dirName + "/" + imageName).st_size:
                    removeFile = False
                else:
    ...
    # Changes the image to the next image in the manifest
    ...
    def classifyImage(self, event):
        ...
        if removeFile:
            os.remove(self.cwd[:-1] + '/' + imageName)
